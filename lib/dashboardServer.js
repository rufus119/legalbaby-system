...
class DashboardServer {
  ...
  loadReportsFromDir(dir, limit, type, filterDate = null) {
    ...
    for (const filename of files) {
      try {
        const data = fs.readFileSync(path.join(resolvedDir, filename), 'utf8');
  ...
  getNextDailyRunIso() {
    ...
    // Scraper runs at 08:30 UTC every day (after Apple midnight Pacific refresh)
  ...
  getNextWeeklyRunIso() {
    const now = new Date();
    const next = new Date(now);
    // Weekly runs at 08:30 UTC every Saturday
    next.setUTCHours(8, 30, 0, 0);
  ...
  serveHTML(res) {
    const isCloudReadOnly = this.isCloudReadOnly();
    const html = `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  <script>
    const CLOUD_READ_ONLY = ${isCloudReadOnly ? 'true' : 'false'};
    ...
    async function runNow(type) {
      if (CLOUD_READ_ONLY) {
        document.getElementById('run-status').innerHTML = 'Cloud mode: data refresh runs from GitHub Actions, not this button.';
        return;
      }
    ...
    async function resetBaseline() {
      if (CLOUD_READ_ONLY) {
        document.getElementById('run-status').innerHTML = 'Cloud mode: reset is disabled. Run reset locally, then push data to GitHub.';
        return;
      }
      const status = document.getElementById('run-status');
    ...
    async function initDashboard() {
      if (CLOUD_READ_ONLY) {
        const runBtnDaily = document.getElementById('run-daily');
        const runBtnWeekly = document.getElementById('run-weekly');
        const resetBtn = document.getElementById('reset-baseline');
        runBtnDaily.disabled = true;
        runBtnWeekly.disabled = true;
        resetBtn.disabled = true;
        document.getElementById('run-status').innerHTML = 'Cloud mode active: this dashboard is read-only. GitHub Actions updates reports automatically.';
      }
      try {
        await loadAvailableDates();
      } catch (error) {
  ...
  serveStatus(res) {
    try {
      ...
      const dailyDirAbs = path.join(process.cwd(), dailyDir);
      if (fs.existsSync(dailyDirAbs)) {
        const files = fs.readdirSync(dailyDirAbs)
          .filter(f => f.endsWith('.json'))
          .sort()
          .reverse();
        ...
        if (files.length > 0) {
          try {
            const data = fs.readFileSync(path.join(dailyDirAbs, files[0]), 'utf8');
            const parsed = JSON.parse(data);
      ...
      const weeklyDirAbs = path.join(process.cwd(), weeklyDir);
      if (fs.existsSync(weeklyDirAbs)) {
        const files = fs.readdirSync(weeklyDirAbs)
          .filter(f => f.endsWith('.json'))
          .sort()
          .reverse();
        ...
        if (files.length > 0) {
          try {
            const data = fs.readFileSync(path.join(weeklyDirAbs, files[0]), 'utf8');
            const parsed = JSON.parse(data);
            if (Array.isArray(parsed) && parsed[0]) {
      ...
      res.end(JSON.stringify({
        ...
        dailyReports: fs.existsSync(dailyDirAbs) ? fs.readdirSync(dailyDirAbs).filter(f => f.endsWith('.json')).length : 0,
        weeklyReports: fs.existsSync(weeklyDirAbs) ? fs.readdirSync(weeklyDirAbs).filter(f => f.endsWith('.json')).length : 0,
  ...
  async runNowDaily(res) {
    if (this.isCloudReadOnly()) {
      res.writeHead(403);
      res.end(JSON.stringify({ error: 'Disabled in cloud mode. Use GitHub Actions for scheduled updates.' }));
      return;
    }
  ...
  async runResetBaseline(res) {
    if (this.isCloudReadOnly()) {
      res.writeHead(403);
      res.end(JSON.stringify({ error: 'Disabled in cloud mode. Run reset locally and push the generated data.' }));
      return;
    }
