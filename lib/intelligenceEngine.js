...
class IntelligenceEngine {
  ...
  detectChanges(oldTracks, newTracks, playlistName = '') {
    ...
    if (changes.moved.length > 0) {
      ...
      // Remove songs already in the right relative order (LCS),
      // then remove micro-movements (< 5 positions) which are not actionable,
      // and sort by magnitude so the biggest chart jumps appear first.
      changes.moved = changes.moved
        .filter(track => !stableSet.has(this.getTrackKey(track)))
        .filter(track => Math.abs(track.to - track.from) >= 5)
