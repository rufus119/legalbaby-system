...
class IntelligenceEngine {
  ...
  detectChanges(oldTracks, newTracks, playlistName = '') {
    ...
    // Filter movements to only TRUE chart shifts — songs that changed position
    // relative to other surviving songs, not just cascading shifts caused by adds/removes.
    if (changes.moved.length > 0) {
      ...
      const oldSurvivorRank = new Map(survivorOldOrder.map((key, i) => [key, i]));
      const newSurvivorRank = new Map(survivorNewOrder.map((key, i) => [key, i]));

      changes.moved = changes.moved.filter(track => {
        const key = this.getTrackKey(track);
        return oldSurvivorRank.get(key) !== newSurvivorRank.get(key);
      });
    }
